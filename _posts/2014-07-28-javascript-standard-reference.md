...
> 1、先调用自身的'valueOf()'方法，如果该方法返回原始值类型，则直接对该值使用Number方法，不再进行后续步骤；
> 2、如果'valueOf()'返回复合类型，再调用对象自身的'toString()'方法，如果'toString()'返回原始值类型，则对该值使用Number，不再进行后续步骤；
> 3、如果'toString()'返回的类型为复合类型，则报错。
